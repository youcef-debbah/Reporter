...
fun <V: Any> MutableMap<String, V>.putValue(value: V): V? = put(value.toString(), value)

fun <V: Any, K : Comparable<K>> NavigableMap<K, V>.closedRange(range: ClosedRange<K>): NavigableMap<K, V> =
    subMap(range.start, true, range.endInclusive, true)

@ExperimentalStdlibApi
fun <V: Any, K : Comparable<K>> NavigableMap<K, V>.openEndRange(range: OpenEndRange<K>): NavigableMap<K, V> =
    subMap(range.start, true, range.endExclusive, false)

fun <K: Any, V: Any> concurrentMap(init: Int): ConcurrentMap<K, V> =
